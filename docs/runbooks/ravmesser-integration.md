...
> **עדכון 15.9 — רב מסר אינו חובה יותר.** תפוצות המייל של ה-CRM עוברות דרך Resend כברירת מחדל
> (`crm_config.email_channel.provider = 'resend'`, מיגרציה 130). המסמך הזה רלוונטי רק אם חוזרים
> לרב מסר: הגדרות → ערוץ מייל → ספק: רב מסר, ואז צריך את ארבעת הסודות שלמטה ואת webhook ההסרה
> ("כיוון 3"). במסלול Resend ההסרה מנוהלת אצלנו — קישור חתום בכל מייל (`email-unsubscribe`).
