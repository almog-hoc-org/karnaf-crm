...
//
// Two shapes:
//   GET /api/healthz           → liveness: returns 200 with version + git SHA.
//   GET /api/healthz?deep=1    → also pings Supabase REST to confirm
//                                downstream reachability + measures latency.
//
// Designed to be a cheap probe target for uptime monitors and load
// balancers. No auth required — payload contains no secrets. Cache-Control
// no-store so monitors never see stale 200s after an outage starts.
...
interface HealthPayload {
  ...
  gitSha: string | null;
  region: string | null;
  uptime?: never;          // edge runtime has no process uptime concept
  checked: Record<string, { ok: boolean; latencyMs?: number; error?: string }>;
  generatedAt: string;
}

function readEnv(name: string): string | undefined {
  // Vercel exposes both formats; prefer process.env (Node-compatible).
...
async function pingSupabase(): Promise<{ ok: boolean; latencyMs: number; error?: string }> {
  ...
  try {
    // /rest/v1/ is the lightest endpoint Supabase exposes — returns the
    // PostgREST OpenAPI banner. Anon key is required; no DB row touched.
