...
- **אישור דיוור ללידים מדפי נחיתה (14.9)** — החלטת הבעלים: כל מי שמגיע מדף נחיתה מאשר דיוור. מיגרציה 126: backfill של `consent_email=true` לכל ליד עם `source='landing_page'` (או `source_detail` = slug של דף נחיתה) שלא היה לו ערך, אירוע `consent_granted` לכל ליד, וטריגר על insert ללידים חדשים. `false` מפורש לא נדרס; `consent_whatsapp` לא נגעו. טקסט הטופס בדף הנחיתה עודכן ("...וקבלת דיוור במייל"). כלל רב מסר `b20` עדיין כבוי וללא list_id — הסכמה לבד לא מוסיפה לרשימה.
