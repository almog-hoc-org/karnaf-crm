...
### ⏳ ממתין לפריסה — סבב "מינימום לחיצות" (2026-08-13, ענף `claude/campaign-broadcasts-handoff-elew7z`)

הסבב הוסיף מענה אינליין מהאינבוקס, סיווג מהיר, קיצורי מקלדת ומיון ברשימת הלידים. שני צעדי פריסה נדרשים, בסדר הזה:

1. **מיגרציה `119_attention_inbox_v5.sql`** — `supabase db push`. מוסיפה שתי עמודות בסוף ה-RPC (`last_inbound_conversation_id`, `last_inbound_channel`). שינוי append-only: `daily-sales-inbox` ו-`dashboard-summary` קוראים עמודות לפי שם וממשיכים לעבוד. עד שהמיגרציה רצה, כפתור "השב כאן" פשוט לא מופיע (fallback מובנה) — אין מצב שבור.
2. **פריסת ה-edge function `leads-list`** — נוסף פרמטר `sort` (whitelist). פרונט ישן מול פונקציה חדשה ולהפך — תקינים.

בדיקה אחרי פריסה: כרטיס בליין "לענות עכשיו" מציג "השב כאן 💬", וה-digest היומי בטלגרם ממשיך להגיע.
