...
- **מעבר ל-Resend בתפוצות מייל (15.9)** — `broadcast-dispatch` היה מקודד לרב מסר והתעלם מ-`crm_config.email_channel.provider`, ולכן שינוי ההגדרה ל-`resend` לא עשה כלום. עכשיו: `_shared/email-channel.ts` (קורא provider + preflight אחד לתזמון ולשליחה), `_shared/resend.ts` (שליחה פר-נמען עם Idempotency-Key, סיווג 429/5xx כזמני), `_shared/email-unsubscribe.ts` + פונקציה ציבורית `email-unsubscribe` (טוקן HMAC לכל ליד, GET דף אישור בעברית, POST one-click לפי RFC 8058, `applyOptOut` scope email), קישור הסרה בכל מייל שיווקי + כותרות `List-Unsubscribe`, עמודה `broadcast_recipients.provider_ref` (מזהה ההודעה ב-Resend), כרטיס "ערוץ מייל" בהגדרות (ספק / שם שולח / כתובת שולח / reply-to). מיגרציה 130 מעבירה את הספק ל-resend ומעתיקה את הג'ימייל ל-replyTo. **חסום עד שהבעלים יאמת דומיין ב-resend.com/domains ויכתוב כתובת שולח עליו** — תזמון תפוצת מייל נחסם בכוונה עד אז (קוד `resend_domain_not_verified`). החבילה החינמית: 100 מיילים ביום, ל-311 צריך חבילה בתשלום.
